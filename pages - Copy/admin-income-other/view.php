<?php
    ...
    if( $search!="" ) {
        $condition_search .= " AND (
            ip.item_income_other_name LIKE '%".$search."%'
        )";
    }
    ...
    $sql = "
        SELECT
            ip.*
        FROM item_income_other ip
        WHERE 1=1
            ".$condition_search."
        ORDER BY ip.item_income_other_id DESC
    ";
    $show = $GLOBAL["SHOW"];
...
<div id="content-body">
    <div class="mb-4">
        <button id="btn-add" class="btn btn-success" title="เพิ่มแหล่งรายได้">
            <i class="fas fa-plus"></i> เพิ่มแหล่งรายได้
        </button>
    </div>
    <div class="row mb-3">
        <div class="col">
            <form id="frm-search" autocomplete="off">
                ...
                <small class="form-text text-muted mb-3">ค้นหา โดยระบุ ชื่อแหล่งรายได้ และกด Enter</small>
            </form>
        </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">ชื่อแหล่งรายได้</th>
                    <th scope="col"></th>
                </tr>
            </thead>
            <tbody>
                    ...
                    foreach($objData as $key=>$row) {
                        echo '
                            <tr data-json="'.htmlspecialchars(json_encode($row)).'">
                                ...
                                <td>'.$row["item_income_other_name"].'</td>
...
<div id="template" class="d-none">
    <form autocomplete="off">
        <input type="hidden" name="item_income_other_id" value="">
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <td class="pt-3" style="min-width: 120px; width: 120px;">แหล่งรายได้</td>
                        <td class="p-2" style="min-width: 200px;">
                            <input type="text" class="form-control" id="item_income_other_name" name="item_income_other_name"
                                required>
                        </td>
                    </tr>
                </tbody>
