$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มโรคติดต่อที่เคยเจ็บป่วยใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขโรคติดต่อที่เคยเจ็บป่วย\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_contagious_id']").val(data.item_contagious_id);
        $contents.find("[name='item_contagious_name']").val(data.item_contagious_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_contagious_id: data.item_contagious_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
