$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มการใช้ประโยชน์พื้นที่ใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขการใช้ประโยชน์พื้นที่\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_area_utilization_id']").val(data.item_area_utilization_id);
        $contents.find("[name='item_area_utilization_name']").val(data.item_area_utilization_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_area_utilization_id: data.item_area_utilization_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
