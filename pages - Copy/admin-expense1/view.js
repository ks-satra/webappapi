$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มค่าใช้จ่ายเพื่อการบริโภคใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขค่าใช้จ่ายเพื่อการบริโภค\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_expense1_id']").val(data.item_expense1_id);
        $contents.find("[name='item_expense1_name']").val(data.item_expense1_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_expense1_id: data.item_expense1_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
