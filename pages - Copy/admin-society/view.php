<?php
    ...
    if( $search!="" ) {
        $condition_search .= " AND (
            ip.item_society_name LIKE '%".$search."%'
        )";
    }
    ...
    $sql = "
        SELECT
            ip.*
        FROM item_society ip
        WHERE 1=1
            ".$condition_search."
        ORDER BY ip.item_society_id DESC
    ";
    $show = $GLOBAL["SHOW"];
...
<div id="content-title">
    ข้อมูลสุขภาวะด้านสภาพทางสังคมของชุมชน
</div>
<div id="content-body">
    <div class="mb-4">
        <button id="btn-add" class="btn btn-success" title="เพิ่มสุขภาวะด้านสภาพทางสังคมของชุมชน">
            <i class="fas fa-plus"></i> เพิ่มสุขภาวะด้านสภาพทางสังคมของชุมชน
        </button>
    </div>
    <div class="row mb-3">
        <div class="col">
            <form id="frm-search" autocomplete="off">
                ...
                <small class="form-text text-muted mb-3">ค้นหา โดยระบุ ชื่อสุขภาวะด้านสภาพทางสังคมของชุมชน และกด Enter</small>
            </form>
        </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">ชื่อสุขภาวะด้านสภาพทางสังคมของชุมชน</th>
                    <th scope="col"></th>
                </tr>
            </thead>
            <tbody>
                <?php
                    ...
                    foreach($objData as $key=>$row) {
                        $item_society_name = mb_strimwidth($row["item_society_name"],0,40,'...');
                        // <td>'.$row["item_society_name"].'</td>
                        echo '
                            <tr data-json="'.htmlspecialchars(json_encode($row)).'">
                                <th class="text-center order">'.(($show*($p-1))+($key+1)).'</th>
                                <td>'.$item_society_name.'</td>
...
<div id="template" class="d-none">
    <form autocomplete="off">
        <input type="hidden" name="item_society_id" value="">
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <td class="pt-3" style="min-width: 250px; width: 250px;">สุขภาวะด้านสภาพทางสังคมของชุมชน</td>
                        <td class="p-2" style="min-width: 200px;">
                            <input type="text" class="form-control" id="item_society_name" name="item_society_name"
                                required>
                        </td>
                    </tr>
                </tbody>
