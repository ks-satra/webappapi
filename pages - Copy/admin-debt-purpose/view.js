$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มวัตถุประสงค์ในการกู้ใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขวัตถุประสงค์ในการกู้\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_debt_purpose_id']").val(data.item_debt_purpose_id);
        $contents.find("[name='item_debt_purpose_name']").val(data.item_debt_purpose_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_debt_purpose_id: data.item_debt_purpose_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
