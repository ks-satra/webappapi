<?php
    ...
    if( $search!="" ) {
        $condition_search .= " AND (
            ip.item_debt_purpose_name LIKE '%".$search."%'
        )";
    }
    ...
    $sql = "
        SELECT
            ip.*
        FROM item_debt_purpose ip
        WHERE 1=1
            ".$condition_search."
        ORDER BY ip.item_debt_purpose_id DESC
    ";
    $show = $GLOBAL["SHOW"];
...
<div id="content-title">
    ข้อมูลวัตถุประสงค์ในการกู้
</div>
<div id="content-body">
    <div class="mb-4">
        <button id="btn-add" class="btn btn-success" title="เพิ่มวัตถุประสงค์ในการกู้">
            <i class="fas fa-plus"></i> เพิ่มวัตถุประสงค์ในการกู้
        </button>
    </div>
    <div class="row mb-3">
        <div class="col">
            <form id="frm-search" autocomplete="off">
                ...
                <small class="form-text text-muted mb-3">ค้นหา โดยระบุ ชื่อวัตถุประสงค์ในการกู้ และกด Enter</small>
            </form>
        </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">ชื่อวัตถุประสงค์ในการกู้</th>
                    <th scope="col"></th>
                </tr>
            </thead>
            <tbody>
                    ...
                    foreach($objData as $key=>$row) {
                        echo '
                            <tr data-json="'.htmlspecialchars(json_encode($row)).'">
                                ...
                                <td>'.$row["item_debt_purpose_name"].'</td>
...
<div id="template" class="d-none">
    <form autocomplete="off">
        <input type="hidden" name="item_debt_purpose_id" value="">
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <td class="pt-3" style="min-width: 150px; width: 150px;">วัตถุประสงค์ในการกู้</td>
                        <td class="p-2" style="min-width: 200px;">
                            <input type="text" class="form-control" id="item_debt_purpose_name" name="item_debt_purpose_name"
                                required>
                        </td>
                    </tr>
                </tbody>
