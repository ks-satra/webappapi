$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มรายการทรัพย์สิน\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขรายการทรัพย์สิน\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_asset_id']").val(data.item_asset_id);
        $contents.find("[name='item_asset_name']").val(data.item_asset_name);
        $contents.find("[name='unit']").val(data.unit);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_asset_id: data.item_asset_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
