$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มแหล่งน้ำที่ใช้ในแปลงใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขแหล่งน้ำที่ใช้ในแปลง\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_area_water_id']").val(data.item_area_water_id);
        $contents.find("[name='item_area_water_name']").val(data.item_area_water_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_area_water_id: data.item_area_water_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
