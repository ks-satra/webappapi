$(function () {
        ...
        ShowConfirm({
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "pages/" + PAGE + "/api/edit-tab19.php",
                        dataType: "JSON",
                        ...
                        success: function (res) {
                            ...
                            if (res.status) {
                                UnbindUnload();
                                LinkTo("./?page=" + PAGE + "&home_id=" + res.home_id + "&tab=20");
                            } else {
                                ShowAlert({
                                    html: res.message,
    ...
    $("#btn-add").click(function () {
        ...
        $contents.find('#unit_id').change(function () {
            var ExtUnit = JSON.parse($("#ExtUnit").val());
            var unit_id = $contents.find('#unit_id').val();
            $contents.find('.unit_id').html(ExtUnit[unit_id]);
        });
        $contents.find('#unit_id').trigger("change");
        ...
        $contents.find('form').submit(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "pages/" + PAGE + "/api/edit-tab19-add.php",
                dataType: "JSON",
    ...
    $(".btn-edit").click(function () {
        ...
        $contents.find("[name='item_animal_water_id']").val(data.item_animal_water_id);
        $contents.find("[name='item_product_water_id']").val(data.item_product_water_id);
        $contents.find("[name='unit_id']").val(data.unit_id);
        ...
        // console.log(data);
        $.each(data.income7_market, function (i, v) {
        ...
        $contents.find('form').submit(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "pages/" + PAGE + "/api/edit-tab19-edit.php",
                dataType: "JSON",
                data: GetFormData(_this),
                contentType: false,
                processData: false,
    ...
    $(".btn-del").click(function () {
        ...
        ShowConfirm({
            ...
            callback: function (rs) {
                if (rs) {
                    ShowLoading();
                    $.post("pages/" + PAGE + "/api/edit-tab19-del.php", {
