<?php
    $sql = "
        SELECT 
            i.*,
            ial.item_animal_land_name
        FROM income6 i
            INNER JOIN item_animal_land ial ON ial.item_animal_land_id=i.item_animal_land_id
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="informant" id="first-scroll">
                <i class="fas fa-folder mr-2"></i> รายได้จากการทำปศุสัตว์
            </div>

            <div>
                <?php
                    foreach($ObjData as $key=>$row) {
                        $row["income6_market"] = $DATABASE->QueryObj("SELECT * FROM income6_market WHERE home_id='".$home['home_id']."' AND order_id='".$row["order_id"]."' ");
                ?>
                <table class="table table-bordered table-sm"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td scope="col">
                                <?php echo $row["item_animal_land_id"]; ?>.
                                <?php echo $row["item_animal_land_name"]; ?>
                            </td>
                        </tr>
                        <tr>
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="p-2" colspan="2">
                            <select class="form-control form-control-sm" id="item_animal_land_id"
                                name="item_animal_land_id" required>
                                <?php
                                    $sql = "SELECT * FROM item_animal_land ORDER BY item_animal_land_id";
                                    $obj = $DATABASE->QueryObj($sql);
                                    foreach($obj as $row) {
                                        echo '<option value="'.$row["item_animal_land_id"].'">'.$row["item_animal_land_id"].'. '.$row["item_animal_land_name"].'</option>';
                                    }
                                ?>
                            </select>
                        </td>
