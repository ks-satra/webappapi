<?php
    $sql = "
        SELECT 
            i.*,
            iv.item_vegetable_name
        FROM income2 i
            INNER JOIN item_vegetable iv ON iv.item_vegetable_id=i.item_vegetable_id
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="informant" id="first-scroll">
                <i class="fas fa-folder mr-2"></i> รายได้จากการปลูกพืชผักสวนครัว/พืชอายุสั้น (ลดรายจ่ายและเชิงพาณิชย์)
            </div>

            <div>
                <?php
                    foreach($ObjData as $key=>$row) {
                        $row["income2_market"] = $DATABASE->QueryObj("SELECT * FROM income2_market WHERE home_id='".$home['home_id']."' AND order_id='".$row["order_id"]."' ");
                ?>
                <table class="table table-bordered table-sm"
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th scope="col">ชนิดของพืชผักสวนครัว/พืชอายุสั้น</th>
                            <td scope="col">
                                <?php echo $row["item_vegetable_id"]; ?>.
                                <?php echo $row["item_vegetable_name"]; ?>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <th scope="col">รายได้รวมจากการปลูกพืชผัก</th>
                            <td scope="col">
                                <?php echo number_format($row["income"],0); ?> บาท/ปี
                            </td>
                        </tr>
                        <tr>
                            <th scope="col">ต้นทุนการปลูกพืชผักโดยรวม</th>
                            <td scope="col">
    ...
    <form autocomplete="off">
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr>
                        <td>ชนิดของพืชผักสวนครัว/พืชอายุสั้น</td>
                        <td class="p-2" colspan="2">
                            <select class="form-control form-control-sm" id="item_vegetable_id" name="item_vegetable_id"
                                required>
                                <?php
                                    $sql = "SELECT * FROM item_vegetable ORDER BY item_vegetable_id";
                                    $obj = $DATABASE->QueryObj($sql);
                                    foreach($obj as $row) {
                                        echo '<option value="'.$row["item_vegetable_id"].'">'.$row["item_vegetable_id"].'. '.$row["item_vegetable_name"].'</option>';
                                    }
                                ?>
                            </select>
                        </td>
                    ...
                    <tr>
                        <td>รายได้รวมจากการปลูกพืชผัก</td>
                        <td class="p-2">
                    ...
                    <tr>
                        <td>ต้นทุนการปลูกพืชผักโดยรวม</td>
                        <td class="p-2">
