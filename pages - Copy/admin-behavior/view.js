$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มการปฏิบัติตัวเริ่มต้นใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขการปฏิบัติตัวเริ่มต้น\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_behavior_id']").val(data.item_behavior_id);
        $contents.find("[name='item_behavior_name']").val(data.item_behavior_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_behavior_id: data.item_behavior_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
