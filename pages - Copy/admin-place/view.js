$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มสถานที่ทำงาน/เรียนใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขสถานที่ทำงาน/เรียน\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_place_name']").val(data.item_place_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_place_id: data.item_place_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
