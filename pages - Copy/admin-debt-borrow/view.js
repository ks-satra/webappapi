$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มตัวเลือกแหล่งกู้ยืมใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขตัวเลือกแหล่งกู้ยืม\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_debt_borrow_id']").val(data.item_debt_borrow_id);
        $contents.find("[name='item_debt_borrow_name']").val(data.item_debt_borrow_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_debt_borrow_id: data.item_debt_borrow_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
