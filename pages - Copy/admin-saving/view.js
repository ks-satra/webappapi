$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มประเภทเงินออมใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขประเภทเงินออม\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_saving_id']").val(data.item_saving_id);
        $contents.find("[name='item_saving_name']").val(data.item_saving_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_saving_id: data.item_saving_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
