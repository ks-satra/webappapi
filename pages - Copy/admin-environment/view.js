$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มสุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชนใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขสุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชน\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_environment_id']").val(data.item_environment_id);
        $contents.find("[name='item_environment_name']").val(data.item_environment_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_environment_id: data.item_environment_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
