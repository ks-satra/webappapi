<?php
    ...
    if( $search!="" ) {
        $condition_search .= " AND (
            ip.item_environment_name LIKE '%".$search."%'
        )";
    }
    ...
    $sql = "
        SELECT
            ip.*
        FROM item_environment ip
        WHERE 1=1
            ".$condition_search."
        ORDER BY ip.item_environment_id DESC
    ";
    $show = $GLOBAL["SHOW"];
...
<div id="content-title">
    ข้อมูลสุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชน
</div>
<div id="content-body">
    <div class="mb-4">
        <button id="btn-add" class="btn btn-success" title="เพิ่มสุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชน">
            <i class="fas fa-plus"></i> เพิ่มสุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชน
        </button>
    </div>
    <div class="row mb-3">
        <div class="col">
            <form id="frm-search" autocomplete="off">
                ...
                <small class="form-text text-muted mb-3">ค้นหา โดยระบุ ชื่อสุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชน และกด Enter</small>
            </form>
        </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">ชื่อสุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชน</th>
                    <th scope="col"></th>
                </tr>
            </thead>
            <tbody>
                <?php
                    ...
                    foreach($objData as $key=>$row) {
                        $item_environment_name = mb_strimwidth($row["item_environment_name"],0,40,'...');
                        echo '
                            <tr data-json="'.htmlspecialchars(json_encode($row)).'">
                                <th class="text-center order">'.(($show*($p-1))+($key+1)).'</th>
                                <td>'.$item_environment_name.'</td>
...
<div id="template" class="d-none">
    <form autocomplete="off">
        <input type="hidden" name="item_environment_id" value="">
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <td class="pt-3" style="min-width: 250px; width: 250px;">สุขภาวะด้านสภาพสิ่งแวดล้อมในชุมชน</td>
                        <td class="p-2" style="min-width: 200px;">
                            <input type="text" class="form-control" id="item_environment_name" name="item_environment_name"
                                required>
                        </td>
                    </tr>
                </tbody>
