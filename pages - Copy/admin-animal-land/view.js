$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มชนิดสัตว์บกใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขชนิดสัตว์บก\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_animal_land_name']").val(data.item_animal_land_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_animal_land_id: data.item_animal_land_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
