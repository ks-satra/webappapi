$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มบันทึกการยืมอุปกรณ์ใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("#imagef").change(function () {
            FileChange(GLOBAL.ALLOW_IMAGE, GLOBAL.ALLOW_SIZE, this, $contents.find("#image"), './files/device_lend/default.png', function () { });
        });
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขบันทึกการยืมอุปกรณ์\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='device_lend_date']").val(data.device_lend_date);
        $contents.find("[name='device_lend_time']").val(data.device_lend_time);
        $contents.find("[name='device_lend_no']").val(data.device_lend_no);
        $contents.find("[name='student_id']").val(data.student_id);
        $contents.find("[name='device_id']").val(data.device_id);
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        device_lend_id: data.device_lend_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
