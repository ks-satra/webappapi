$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มแหล่งเงินช่วยเหลือใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขแหล่งเงินช่วยเหลือ\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_help_id']").val(data.item_help_id);
        $contents.find("[name='item_help_name']").val(data.item_help_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_help_id: data.item_help_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
