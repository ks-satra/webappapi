<?php
    ...
    $DATABASE->QueryDelete("expense3", "home_id='".$data["home_id"]."'");
    foreach($data["amount"] as $key=>$row) {
        ...
        $DATABASE->QueryInsert("expense3", $field);
    }

    $step = 30; $DATABASE->QueryDelete("form_step", " home_id='".$data["home_id"]."' AND step='".$step."' "); $DATABASE->QueryInsert("form_step", array("home_id"=>$data["home_id"], "step"=>$step));
    echo json_encode(array(
        "status"=>true,
