<?php
    $sql = "
        SELECT 
            ...
            `if`.item_farm_name
        FROM income3 i
            INNER JOIN item_farm `if` ON `if`.item_farm_id=i.item_farm_id
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="informant" id="first-scroll">
                <i class="fas fa-folder mr-2"></i> รายได้จากการปลูกพืชไร่
            </div>

            <div>
                <?php
                    foreach($ObjData as $key=>$row) {
                        $row["income3_market"] = $DATABASE->QueryObj("SELECT * FROM income3_market WHERE home_id='".$home['home_id']."' AND order_id='".$row["order_id"]."' ");
                ?>
                <table class="table table-bordered table-sm"
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th scope="col">ชนิดของพืชไร่</th>
                            <td scope="col">
                                <?php echo $row["item_farm_id"]; ?>.
                                <?php echo $row["item_farm_name"]; ?>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <th scope="col">รายได้รวมการปลูกพืชไร่</th>
                            <td scope="col">
                                <?php echo number_format($row["income"],0); ?> บาท/ปี
                            </td>
                        </tr>
                        <tr>
                            <th scope="col">ต้นทุนการปลูกพืชไร่โดยรวม</th>
                            <td scope="col">
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="p-2" colspan="2">
                            <select class="form-control form-control-sm" id="item_farm_id" name="item_farm_id" required>
                                <?php
                                    $sql = "SELECT * FROM item_farm ORDER BY item_farm_id";
                                    $obj = $DATABASE->QueryObj($sql);
                                    foreach($obj as $row) {
                                        echo '<option value="'.$row["item_farm_id"].'">'.$row["item_farm_id"].'. '.$row["item_farm_name"].'</option>';
                                    }
                                ?>
                            </select>
                        </td>
                    ...
                    <tr>
                        <td>ต้นทุนการปลูกพืชไร่โดยรวม</td>
                        <td class="p-2">
