<?php
    $sql = "
        SELECT 
            ...
            e.unit_id,
            e.total
        FROM item_expense2 ie
            LEFT JOIN expense2 e ON e.item_expense2_id=ie.item_expense2_id 
                AND e.home_id='".$home['home_id']."'
        ORDER BY ie.item_expense2_id
    ";
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="informant" id="first-scroll">
                <i class="fas fa-folder mr-2"></i> ค่าใช้จ่ายเพื่อการอุปโภค
            </div>
                    ...
                    <tbody>
                        <?php
                            foreach($ObjData as $key=>$row) {
                                ...
                                echo '
                                    <tr>
                                        <td>'.($key+1).'. '.$row["item_expense2_name"].'</td>
                                        <td class="p-2"><input type="tel" class="form-control form-control-sm" name="amount['.$row["item_expense2_id"].']" value="'.$amount.'" data-input-number="0"></td>
                                        <td class="p-2">
                                            <select class="form-control form-control-sm" name="unit_id['.$row["item_expense2_id"].']">
                                        ...
                                        <td class="p-2"><input type="tel" class="form-control form-control-sm text-right" name="total['.$row["item_expense2_id"].']" value="'.number_format($total,0).'" disabled></td>
                                    </tr>
                                ';
                            }
                        ?>
