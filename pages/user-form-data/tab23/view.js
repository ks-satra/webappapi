$(function () {
        ...
        ShowConfirm({
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "pages/" + PAGE + "/api/edit-tab23.php",
                        dataType: "JSON",
                        ...
                        success: function (res) {
                            ...
                            if (res.status) {
                                UnbindUnload();
                                LinkTo("./?page=" + PAGE + "&home_id=" + res.home_id + "&tab=24");
                            } else {
                                ShowAlert({
                                    html: res.message,
    ...
    $("#btn-add").click(function () {
        ...
        ToggleInputNumber($contents.find('#kg3'), 0);
        ToggleInputNumber($contents.find('#cost'), 0);
        $contents.find('#kg1, #kg2, #kg3, #cost').keyup(function () {
            var kg1 = ToNum($contents.find('#kg1').val()) * 1;
            var kg2 = ToNum($contents.find('#kg2').val()) * 1;
            var kg3 = ToNum($contents.find('#kg3').val()) * 1;
        ...
        $contents.find('form').submit(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "pages/" + PAGE + "/api/edit-tab23-add.php",
                dataType: "JSON",
    ...
    $(".btn-edit").click(function () {
        ...
        $contents.find("[name='type_name']").val(data.type_name);
        ...
        ToggleInputNumber($contents.find('#kg2'), 0);
        ToggleInputNumber($contents.find('#cost'), 0);
        $contents.find('#kg1, #kg2, #cost').keyup(function () {
            var kg1 = ToNum($contents.find('#kg1').val()) * 1;
            var kg2 = ToNum($contents.find('#kg2').val()) * 1;
            var cost = ToNum($contents.find('#cost').val()) * 1;
            var income = kg1 + kg2;
        ...
        $contents.find('form').submit(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "pages/" + PAGE + "/api/edit-tab23-edit.php",
                dataType: "JSON",
                data: GetFormData(_this),
                contentType: false,
                processData: false,
    ...
    $(".btn-del").click(function () {
        ...
        ShowConfirm({
            ...
            callback: function (rs) {
                if (rs) {
                    ShowLoading();
                    $.post("pages/" + PAGE + "/api/edit-tab23-del.php", {
