<?php
    ...
    if( $search!="" ) {
        $condition_search .= " AND (
            ip.item_occupation_name LIKE '%".$search."%'
        )";
    }
    ...
    $sql = "
        SELECT
            ip.*
        FROM item_occupation ip
        WHERE 1=1
            ".$condition_search."
        ORDER BY ip.item_occupation_id DESC
    ";
    $show = $GLOBAL["SHOW"];
...
<div id="content-body">
    <div class="mb-4">
        <button id="btn-add" class="btn btn-success" title="เพิ่มอาชีพ">
            <i class="fas fa-plus"></i> เพิ่มอาชีพ
        </button>
    </div>
    <div class="row mb-3">
        <div class="col">
            <form id="frm-search" autocomplete="off">
                ...
                <small class="form-text text-muted mb-3">ค้นหา โดยระบุ ชื่ออาชีพ และกด Enter</small>
            </form>
        </div>
                    ...
                    foreach($objData as $key=>$row) {
                        echo '
                            <tr data-json="'.htmlspecialchars(json_encode($row)).'">
                                ...
                                <td>'.$row["item_occupation_name"].'</td>
...
<div id="template" class="d-none">
    <form autocomplete="off">
        <input type="hidden" name="item_occupation_id" value="">
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <td class="pt-3" style="min-width: 85px; width: 85px;">อาชีพ</td>
                        <td class="p-2" style="min-width: 200px;">
                            <input type="text" class="form-control" id="item_occupation_name" name="item_occupation_name"
                                required>
                        </td>
                    </tr>
                </tbody>
