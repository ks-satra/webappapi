$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มชนิดของพืชผักสวนครัว/พืชอายุสั้นใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขชนิดของพืชผักสวนครัว/พืชอายุสั้น\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_vegetable_name']").val(data.item_vegetable_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_vegetable_id: data.item_vegetable_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
