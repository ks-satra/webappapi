<?php
    ...
    if( $search!="" ) {
        $condition_search .= " AND (
            ip.item_contagious_name LIKE '%".$search."%'
        )";
    }
    ...
    $sql = "
        SELECT
            ip.*
        FROM item_contagious ip
        WHERE 1=1
            ".$condition_search."
        ORDER BY ip.item_contagious_id DESC
    ";
    $show = $GLOBAL["SHOW"];
...
<div id="content-title">
    ข้อมูลโรคติดต่อที่เคยเจ็บป่วย
</div>
<div id="content-body">
    <div class="mb-4">
        <button id="btn-add" class="btn btn-success" title="เพิ่มโรคติดต่อที่เคยเจ็บป่วย">
            <i class="fas fa-plus"></i> เพิ่มโรคติดต่อที่เคยเจ็บป่วย
        </button>
    </div>
    <div class="row mb-3">
        <div class="col">
            <form id="frm-search" autocomplete="off">
                ...
                <small class="form-text text-muted mb-3">ค้นหา โดยระบุ ชื่อโรคติดต่อที่เคยเจ็บป่วย และกด Enter</small>
            </form>
        </div>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">ชื่อโรคติดต่อที่เคยเจ็บป่วย</th>
                    <th scope="col"></th>
                </tr>
            </thead>
            <tbody>
                    ...
                    foreach($objData as $key=>$row) {
                        echo '
                            <tr data-json="'.htmlspecialchars(json_encode($row)).'">
                                ...
                                <td>'.$row["item_contagious_name"].'</td>
...
<div id="template" class="d-none">
    <form autocomplete="off">
        <input type="hidden" name="item_contagious_id" value="">
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <td class="pt-3" style="min-width: 170px; width: 170px;">โรคติดต่อที่เคยเจ็บป่วย</td>
                        <td class="p-2" style="min-width: 200px;">
                            <input type="text" class="form-control" id="item_contagious_name" name="item_contagious_name"
                                required>
                        </td>
                    </tr>
                </tbody>
