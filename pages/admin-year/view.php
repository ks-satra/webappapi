...
<div id="content-body">
    <div class="mb-4">
        <button id="btn-add" class="btn btn-success" title="เพิ่มปีงบประมาณ">
            ...
            <tbody>
                <?php
                    ...
                    foreach($objData as $key=>$row) {
                        $status = array(
                            "Y"=>'<span class="text-success">เปินค่าเริ่มต้น</span>',
                            "N"=>'<a href="Javascript:" class="btn-default">ไม่เป็นค่าเริ่มต้น</a>'
                        );
                        echo '
                            <tr data-json="'.htmlspecialchars(json_encode($row)).'">
                                <th class="text-center order">'.(($show*($p-1))+($key+1)).'</th>
                                <td>'.$row["year_name"].'</td>
                                <td>'.$status[$row["default"]].'</td>
    ...
    <form autocomplete="off">
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <td class="pt-3" style="min-width: 100px; width: 100px;">ปีงบประมาณ</td>
                        <td class="p-2" style="min-width: 200px;">
                            <input type="text" class="form-control" id="year_name" name="year_name" maxlength="4"
                                required>
                            <small class="form-text text-muted">เช่น 2564</small>
                        </td>
                    </tr>
                </tbody>
            </table>
