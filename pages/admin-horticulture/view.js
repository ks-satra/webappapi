$(function () {
    ...
    $("#btn-add").click(function () {
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-plus mb-2"></i> เพิ่มชนิดของพืชสวนใหม่\
			</div>\
		');
        $contents = $('\
			<div>\
    ...
    $(".btn-edit").click(function () {
        ...
        $title = $('\
			<div style="font-size: 16px;">\
                <i class="fas fa-edit mb-2"></i> แก้ไขชนิดของพืชสวน\
			</div>\
		');
        $contents = $('\
			<div>\
        ...
        $contents.find("[name='item_horticulture_name']").val(data.item_horticulture_name);
        BindUnload($contents.find('form'));
        $contents.find('form').submit(function (e) {
            e.preventDefault();
            var _this = this;
            ...
            callback: function (rs) {
                if (rs) {
                    ...
                    $.post("pages/" + PAGE + "/api/del.php", {
                        item_horticulture_id: data.item_horticulture_id
                    }, function (res) {
                        HideLoading();
                        ShowAlert({
                            html: res.message,
